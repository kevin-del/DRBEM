% DRBEM sin 3x * (exp(y)-exp(-y)) dengan
% y=0 ======> u=0
...
for n=1:M
    for k=1:N
        ...
        if bt(k)==0
            a(n,k)=-F1(n,k);
            b(n)=b(n)+(0.5*delta-F2(n,k))*bv1(k)-mu(n,k)*g(x(k),y(k));
        else
            a(n,k)=F2(n,k)-0.5*delta;
...
for i=1:M
    An(i)=(x(i))^2 * (y(i))^2; %solusi analitik DRBEM di soal.
    Er(i)=abs(An(i)-real(U(i)));
end


t=[U;Un]; %U: Solusi numerik, Un: Solusi derivatif numerik
fid=fopen('U-dan-Un_Poisson.txt','wt');
...
fid=fopen('Numerik_vs_Analitik_Poisson.txt','wt');
