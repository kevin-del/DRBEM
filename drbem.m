...
for n=1:M
    for k=1:N
        ...
        if bt(k)==0
            a(n,k)=-F1(n,k);
            b(n)=b(n)+(0.5*delta+omega*mu(n,k)-F2(n,k))*bv1(k);
        else
            a(n,k)=F2(n,k)-omega*mu(n,k)-0.5*delta;
            b(n)=b(n)+F1(n,k)*bv2(k);
        end
    end
    for k=N+1:M
        if n==k
...
for i=1:M
    An(i)=sin(3*x(i)) * (exp(y(i))-exp(-(y(i)))); %solusi analitik DRBEM di soal.
    Er(i)=abs(An(i)-real(U(i)));
end


t=[U;Un]; %U: Solusi numerik, Un: Solusi derivatif numerik
fid=fopen('U-dan-Un.txt','wt');
...
fid=fopen('Numerik_vs_Analitik.txt','wt');
